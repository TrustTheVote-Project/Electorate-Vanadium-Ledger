...
export ADMIN_PRIVATE_KEY_ARN=$(aws cloudformation describe-stacks --stack-name $CREDENTIALS_STACK --query 'Stacks[0].Outputs[?OutputKey==`AdminPrivateKeyArn`].OutputValue' --output text)
export ADMIN_SIGNED_CERT_ARN=$(aws cloudformation describe-stacks --stack-name $CREDENTIALS_STACK --query 'Stacks[0].Outputs[?OutputKey==`AdminSignedCertArn`].OutputValue' --output text)
