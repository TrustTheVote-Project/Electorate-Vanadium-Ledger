...
async function invoke(func, args) {
    ...
    const request = {chaincodeId: config.chaincodeName, fcn: func, args: args, chainId: config.channelName};
    ...
    try {
        ...
        if (successfulResponses) {
            ...
            event_hubs.forEach((eh) => {
                ...
                let invokeEventPromise = new Promise((resolve, reject) => {
                    ...
                    eh.registerTxEvent(transactionId.getTransactionID(), (tx, code, block_num) => {
                        ...
                        console.info('##### invokeChaincode - Transaction %s has status of %s in block %s', tx, code, block_num);
            ...
            for (let i in results) {
                ...
                console.info('##### invokeChaincode - Event results for event hub :%s',event_hub.getPeerAddr());
                if (typeof event_hub_result === 'string') {
                    console.info('##### invokeChaincode - ' + event_hub_result);
                } 
                else {
                    if (!error_message) error_message = event_hub_result.toString();
                    console.info('##### invokeChaincode - ' + event_hub_result.toString());
                }
            }
        } 
        else {
